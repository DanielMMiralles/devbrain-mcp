...
class DevBrainHUD:
    ...
    def generate_metrics_panel(self, summary: dict) -> Panel:
        ...
        if t_counts:
            ...
            for tool_name, count in sorted(t_counts.items(), key=lambda x: x[1], reverse=True)[:3]:
                pct = count / max_count
                bars = int(pct * 8)
                bar_str = "█" * bars + "░" * (8 - bars)
                act_table.add_row(
        ...
        return Panel(content, title="[bold]⚡ Métricas de Rendimiento & Consumo de Tokens[/bold]", box=box.ROUNDED, border_style=self.theme.secondary)
    ...
    def generate_cognitive_panel(self, summary: dict) -> Panel:
        ...
        # Rastro de pensamiento con panel sutil
        thought_box = Panel(
        ...
        # Visualización de corteza sináptica en red
        ...
        return Panel(content, title="[bold]💡 ¿Cómo Piensa? (Cognición & Sinapsis)[/bold]", box=box.ROUNDED, border_style=self.theme.primary)
    ...
    def generate_orchestration_panel(self) -> Panel:
        ...
        status_line = Text.assemble(
            ...
            ("│ ", f"dim {self.theme.dim}"),
            ("● gentle-shell v3.4.0 (dock listo) ", f"bold {self.theme.accent}"),
            ("│ ", f"dim {self.theme.dim}"),
        ...
        return Panel(grid, title="[bold]🔄 Carril de Orquestación en Vivo[/bold]", box=box.ROUNDED, border_style=self.theme.accent)
    ...
    def run_watch(self, refresh_rate: float = 1.0) -> None:
        """Modo observador pasivo continuo SIN parpadeo (actualiza solo ante cambios reales)."""
        self.console.clear()
        self.console.print(self.render_layout())
        self.console.print(f"[{self.theme.dim}]👁️ Modo Observador Activo — Monitoreando bus de eventos en segundo plano... (Ctrl+C para salir)[/{self.theme.dim}]\n")
        ...
        try:
            while True:
                ...
                # Solo redibujar si hubo un cambio genuino en las métricas
                if curr_calls != last_calls or curr_tokens != last_tokens:
                    last_calls = curr_calls
                    last_tokens = curr_tokens
                    self.console.clear()
                    self.console.print(self.render_layout())
                    self.console.print(f"[{self.theme.dim}]👁️ Evento detectado ({datetime.now().strftime('%H:%M:%S')}) — Observador activo (Ctrl+C para salir)[/{self.theme.dim}]\n")
